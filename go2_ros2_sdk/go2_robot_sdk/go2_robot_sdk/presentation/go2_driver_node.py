...
class Go2DriverNode(Node):
    ...
    def __init__(self, event_loop=None):
        ...
        self.robot_adapter = self._create_robot_adapter()
    ...
    def _create_robot_adapter(self):
        if self.config.conn_type == 'sim':
            from ..infrastructure.sim import SimAdapter
            return SimAdapter(config=self.config, node=self)
        else:
    ...
    async def connect_robots(self) -> None:
        """Connect to robots"""
        if self.config.conn_type in ('webrtc', 'sim'):
